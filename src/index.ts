...
async function readFileFromGitHub(owner: string, repo: string, path: string, token: string): Promise<string> {
  try {
    ...
    throw new Error(`Error fetching file from GitHub: ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function readFileFromBitbucket(owner: string, repo: string, path: string, username: string, token: string): Promise<string> {
  try {
    const url = `https://api.bitbucket.org/2.0/repositories/${owner}/${repo}/src/master/${encodeURIComponent(path)}`;
    const response = await axios.get(url, {
      auth: {
        username: username,
        password: token,
    ...
    throw new Error(`Error fetching file from Bitbucket: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function evaluateDocumentFromSource(source: 'github' | 'bitbucket', owner: string, repo: string, filePath: string, token: string, username?: string): Promise<void> {
  try {
    let content: string;
    ...
    switch (source) {
      ...
      case 'bitbucket':
        ...
        content = await readFileFromBitbucket(owner, repo, filePath, username, token);
        break;
      default:
...
const source = 'github'; // Change to 'github' as needed
const owner = 'muktar-bs';
const repo = 'docs';
const token = '';
const filePath = 'project-overview.md';
const username = 'tarekhossain'; // Only needed for Bitbucket

// Example usage for bitbucket
// const source = 'bitbucket'; // Change to 'github' as needed
// const owner = 'bs-rezve';
// const repo = 'int-oip';
// const token = '';
